...
const ADMIN_EMAIL = "yeshuv24@gmail.com";
const ADMIN_INITIAL_PASSWORD = "Kalavathi@312";
...
export const initAdminUser = async (): Promise<void> => {
  try {
    ...
    if (existingUser) {
      ...
      const isMatch = await bcrypt.compare(ADMIN_INITIAL_PASSWORD, existingUser.passwordHash);
